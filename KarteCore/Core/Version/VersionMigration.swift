...
internal struct VersionMigration: Migration {
    @Injected(name: "version_service.repository")
    var repository: Repository

    @Injected(name: "version_service.file_url")
    var fileURL: URL

    @Injected(name: "version_service.legacy_file_url")
    ...
    func migrate() throws {
        ...
        guard let version = dict["versionName"] as? String else {
            return
        }

        try repository.write(Version(previous: version))
    }
}
