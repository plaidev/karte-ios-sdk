...
internal struct RetryPubSubSubscriber: TrackingSubscriber {
    var id = PubSubSubscriberId("io.karte.pubsub.subscriber.retry")
    var app: KarteApp
    ...
    func receive(_ messages: [PubSubMessage], topic: PubSubTopic, from deliverer: PubSubDeliverer) {
        ...
        requests.forEach { send($0, from: deliverer) }
    }
}
