...
internal struct TrackEventRejectionFilter {
    ...
    func reject(event: Event) -> Bool {
        ...
        guard let subRuleContainer = ruleContainer[libraryName] else {
            return false
        }
        guard let rules = subRuleContainer[event.eventName.rawValue] else {
            return false
        }
    ...
    mutating func add(rule: TrackEventRejectionFilterRule) {
        ...
        subRuleContainer[rule.eventName.rawValue] = rules
        ruleContainer[rule.libraryName] = subRuleContainer
    }
}
