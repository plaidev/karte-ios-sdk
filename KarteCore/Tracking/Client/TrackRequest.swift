...
public struct TrackRequest: Request {
    ...
    public var bodyParameters: BodyParameters? {
        let events = dataSets.map { dataSet -> Event in
            var event = dataSet.data.event
            event.mergeAdditionalParameter(date: dataSet.data.attributions.date, isRetry: isRetry)
            return event
        }
    ...
    /// - Parameters:
    ///   - app: `KarteApp` インスタンス
    ///   - dataSets: トラッキングデータセット配列
    ///   - isRetry: リトライされたイベントであるか。リトライイベントの場合は `true` となる。
    init?(app: KarteApp, dataSets: [TrackingDataSet], isRetry: Bool) {
        guard let attributions = dataSets.first?.data.attributions, let appInfo = app.appInfo else {
            return nil
        }

        self.visitorId = attributions.visitorId
        self.sceneId = attributions.sceneId
        self.pvId = attributions.pvId
        self.originalPvId = attributions.originalPvId
    ...
    public func contains(eventName: EventName) -> Bool {
        dataSets.contains { dataSet -> Bool in
            let ret = dataSet.data.event.eventName == eventName
            return ret
        }
    }
}
