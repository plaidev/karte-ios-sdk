...
extension Statement {
    ...
    public func dataForColumn(at index: Int32) -> Data? {
        ...
        let bytes = sqlite3_column_blob(statementHandle, index)
        
        return bytes.flatMap({ Data(bytes: $0, count: byteCount) })
    }
    ...
    public func characterForColumn(at index: Int32) -> Character? {
        return stringForColumn(at: index)?.first
    }
