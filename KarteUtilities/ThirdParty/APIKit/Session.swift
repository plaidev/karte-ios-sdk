...
open class Session {
    ...
    @discardableResult
    open class func send<Request: KarteUtilities.Request>(_ request: Request, callbackQueue: CallbackQueue? = nil, handler: @escaping (Result<Request.Response, SessionTaskError>) -> Void = { _ in }) -> SessionTask? {
    ...
    open class func cancelRequests<Request: KarteUtilities.Request>(with requestType: Request.Type, passingTest test: @escaping (Request) -> Bool = { _ in true }) {
    ...
    /// - returns: The new session task.
    @discardableResult
    open func send<Request: KarteUtilities.Request>(_ request: Request, callbackQueue: CallbackQueue? = nil, handler: @escaping (Result<Request.Response, SessionTaskError>) -> Void = { _ in }) -> SessionTask? {
    ...
    open func cancelRequests<Request: KarteUtilities.Request>(with requestType: Request.Type, passingTest test: @escaping (Request) -> Bool = { _ in true }) {
    ...
    private func setRequest<Request: KarteUtilities.Request>(_ request: Request, forTask task: SessionTask) {
        objc_setAssociatedObject(task, &taskRequestKey, request, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
    }

    private func requestForTask<Request: KarteUtilities.Request>(_ task: SessionTask) -> Request? {
